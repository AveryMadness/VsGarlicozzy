...
{				    // custom mod option shit is commented out until i seperate from garlicozzy
	var options:Array<String> = [/*'Mod Name'*/'Garlicozzy', 'Notes', 'Controls', 'Preferences'];
	...
	override function update(elapsed:Float) {
		...
		if (controls.ACCEPT) {
			...
			switch(options[curSelected]) {
				...
				case 'Garlicozzy':
					openSubState(new GarlicozzyOptionsSubstate());
			      /*case 'Mod Name':
					openSubState(new ModNameOptionsSubState()); */
			}
		}
	}
...
class GarlicozzyOptionsSubstate extends MusicBeatSubstate
{
	private static var curSelected:Int = 0;
	static var unselectableOptions:Array<String> = [
		'',
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Unfairness Modchart':
						ClientPrefs.unfairmodchart = !ClientPrefs.unfairmodchart;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			case 'Unfairness Modchart':
				daText = "Toggles Unfairness And Unfairness B-Side Modchart.";
		}
		descText.text = daText;
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					case 'FPS Counter':
						daValue = ClientPrefs.showFPS;
					case 'Unfairness Modchart':
						daValue = ClientPrefs.unfairmodchart;
					case 'Anti-Aliasing':
						daValue = ClientPrefs.globalAntialiasing;
					case 'Note Splashes':
						daValue = ClientPrefs.noteSplashes;
...
/* class ModNameOptionsSubstate extends MusicBeatSubstate
{
	private static var curSelected:Int = 0;
	static var unselectableOptions:Array<String> = [
		'Mod Category',
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'ModOption1':
						ClientPrefs.modoption1 = !ClientPrefs.modoption1;
					case 'ModOption2':
						ClientPrefs.modoption2 = !ClientPrefs.modoption2;
					case 'ModOption3':
						ClientPrefs.modoption3 = !ClientPrefs.modoption3;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			case 'ModOption1':
				daText = "Mod Option 1 Description.";
			case 'ModOption2':
				daText = "Mod Option 2 Description.";
			case 'ModOption3':
				daText = "Mod Option 3 Description.";
		}
		descText.text = daText;
	...
	function reloadValues() {
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					case 'ModOption1':
						daValue = ClientPrefs.modoption1;
					case 'ModOption2':
						daValue = ClientPrefs.modoption2;
					case 'ModOption3':
						daValue = ClientPrefs.modoption3;
				}
				checkbox.daValue = daValue;
			}
		}
